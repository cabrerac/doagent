...
- **`examples/llm_policy.py`**: Add a `create_llm_tool()` helper that reads `OPENAI_API_KEY` (or `DOAGENT_OPENAI_API_KEY`) from env and creates the LLM callable using the OpenAI SDK. Also supports Gemini as an alternative provider.
- **OpenAI SDK** is an optional dependency for examples only — users running heuristic-only demos don't need it. Default model: `gpt-4o`.
- **Error handling**:
