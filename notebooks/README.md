...
**Config alignment:** Push and gridworld notebooks use the same parameters as the local examples (`examples/push/`, `examples/gridworld/config.yaml`) so behaviour matches between Colab and local runs. The gridworld notebook demonstrates **decentralisation** (peer-to-peer topology) and **openness** (join/leave records + `visible_participants`). The run loop matches the local example: `decision_context(..., summarise=build_shared_map)` for the shared map, and `visible_participants` passed into `decide`.
